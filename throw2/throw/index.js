...
function setupModalCloseButtons() {
    // Close buttons for create list modal
    const createListCloseButtons = createListModal.querySelectorAll('[data-modal-hide="create-list-modal"]');
    createListCloseButtons.forEach(button => {
    ...
    // Close buttons for edit list modal
    const editListCloseButtons = editListModal.querySelectorAll('[data-modal-hide="edit-list-modal"]');
    editListCloseButtons.forEach(button => {
        button.addEventListener('click', () => {
            if (window.Modal) {
...
async function handleSearch(event) {
    ...
    try {
        ...
        console.log('API URL:', `${API_BASE_URL}/movieAndtvShow`);
        ...
        showError(`Failed to search: ${error.message}. Please check if json-server is running on port 3000.`);
    }
}
...
function selectItemForList(itemId) {
    ...
    console.log('Selected item:', selectedItemForList);
...
function showCreateListModal() {
    ...
    // Show the modal using Flowbite - check if Modal is available
    if (window.Modal) {
...
async function loadLists() {
    try {
        console.log('Loading lists from:', `${API_BASE_URL}/lists`);
        ...
        console.log('Loaded lists:', currentLists);
        displayLists();
        
    } catch (error) {
        console.error('Error loading lists:', error);
        myListsDiv.innerHTML = `<p class="text-red-500 text-center">Failed to load lists: ${error.message}</p>`;
        // Initialize empty lists array if there's an error
        currentLists = [];
    }
}
